 ...
 * Efectos de energía oscura, animaciones fluidas y diseño impactante
 */
...
export class JujutsuIntro {
    constructor(container) {
        ...
        this.parameters = {
            colorPrimary: 0x8b0000,    // Rojo oscuro (Sukuna)
            colorSecondary: 0x4b0082,  // Morado oscuro
            colorAccent: 0x00ffff,      // Cian eléctrico
            colorDark: 0x050008,
            colorLight: 0xffffff,
            particleCount: 1500,
            particleSize: 0.08,
    ...
    createScene() {
        // Background gradient (simulado con niebla y partículas)
        this.scene.background = new THREE.Color(this.parameters.colorDark);
        
        // Ambient Light - Luz tenebrosa
        const ambientLight = new THREE.AmbientLight(
            this.parameters.colorPrimary,
            0.4
        );
        this.scene.add(ambientLight);
        
        // Directional Light - Luz principal
        const directionalLight = new THREE.DirectionalLight(
            this.parameters.colorAccent,
            1
        );
        ...
        // Point Light - Luz de energía maldita
        const cursedLight = new THREE.PointLight(
    ...
    createParticles() {
        ...
        for (let i = 0; i < this.parameters.particleCount; i++) {
            ...
            // Posiciones en forma de esfera con ruido
            ...
            // Colores variados entre primario, secundario y accent
    ...
    createCursedEnergy() {
        // Crear energía maldita (líneas de energía)
        ...
        spiralGeometry.setAttribute(
            'color',
            new THREE.Float32BufferAttribute(spiralColors, 3)
        );
    ...
    createLogo() {
        ...
        // Crear logo estilo "Maldición" - Símbolo de Sukuna
        // Círculo exterior
        ...
        // Líneas radiales (como los pétalos de la flor de Sukuna)
        for (let i = 0; i < 8; i++) {
            const angle = (i / 8) * Math.PI * 2;
            const lineGeometry = new THREE.BufferGeometry();
            const linePoints = [
                0, 0, 0,
                Math.cos(angle) * 1.8, Math.sin(angle) * 1.8, 0
            ];
        ...
        const centerMaterial = new THREE.MeshBasicMaterial({
            color: this.parameters.colorLight
        });
    ...
    createText() {
        // Crear texto "SENA 3D" con estilo Jujutsu Kaisen
        ...
        const gradient = ctx.createLinearGradient(
            ...
            canvas.width, canvas.height / 2 - 60
        );
            ...
            if (progress < 1) {
                ...
                const animateText = () => {
                    ...
                    if (this.objects.titleText && this.objects.titleText.material) {
                        this.objects.titleText.material.opacity = opacity;
                    }
    ...
    animateCursedLight() {
        ...
        const animate = () => {
            ...
            const intensity = 2 + Math.sin(time * Math.PI * 2) * 3;
            
            if (this.objects.cursedLight) {
                this.objects.cursedLight.intensity = intensity;
            }
